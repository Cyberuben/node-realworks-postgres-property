...
function parseProperty(property) {
	...
	}else if(property.hasOwnProperty("Functies")) {
		returnData.type = "bog";

		if(property.Functies.hasOwnProperty("Winkelruimte")) {
			returnData.propertyType = "winkelruimte";
		}else if(property.Functies.hasOwnProperty("Bedrijfsruimte")) {
			returnData.propertyType = "bedrijfsruimte";
		}else if(property.Functies.hasOwnProperty("Horeca")) {
			returnData.propertyType = "horeca";
		}else if(property.Functies.hasOwnProperty("Kantoorruimte")) {
			returnData.propertyType = "kantoorruimte";
		}else{
			returnData.propertyType = "anders";
		}
	}else{
	...
	if(property.ObjectDetails.Adres.hasOwnProperty("Nederlands")) {
		...
		if(typeof address.Huisnummer == "object" && address.Huisnummer.hasOwnProperty("Hoofdnummer")) {
	...
	if(property.ObjectDetails.hasOwnProperty("Koop")) {
		...
		if(returnData.buyPrefix == "prijs op aanvraag") {
			...
			if(returnData.buyPrefix) {
				returnData.buyPrefix = returnData.buyPrefix.charAt(0).toUpperCase() + returnData.buyPrefix.slice(1);
			}else{
				returnData.buyPrefix = "Vraagprijs";
			}
